...
export async function POST(
  ...
  try {
    ...
    const resolvedParams = await params;  // <-- attendre la Promise
    const patienteId = resolvedParams.id;
    ...
    const donnee = new DonneesCliniques({
      patienteId,
      dateSaisie: new Date(),
      age: body.age,
      BMI: body.BMI,
      ...
      HOMA: body.HOMA,
      leptin: body.leptin,
      adiponectin: body.adiponectin,
      resistin: body.resistin,
      MCP1: body.MCP1,
    });
...
export async function GET(
  ...
  try {
    ...
    // Vérifie que la patiente appartient bien au gynécologue
    const patiente = await Patiente.findOne({
      _id: patienteId,
      gynecoId: user.userId,
    });
    ...
    // Récupère toutes les données cliniques de cette patiente
...
export async function PUT(
  ...
  try {
    ...
    const resolvedParams = await params;
    const donneeId = resolvedParams.id;
    ...
    // Met à jour tous les champs sauf dateSaisie
const champs = [
  "age",
  "BMI",
  "glucose",
...
(champs as (keyof IDonneesCliniques)[]).forEach((c) => {
  if (body[c] !== undefined) (donnee as any)[c] = body[c];
});
